...
export class ProductTableComponent implements OnInit {
  ...
  getProducts(){
    this.errorMessage = '';
    this.http.get<Product[]>(`${this.API_URL}/api/Products`, {headers: {'Authorization': `Bearer ${this.user?.jwt}`}}).subscribe(
      prods => {
        console.log(prods);
        this.products = prods;
  ...
  deleteProduct(id: number){
    ...
    this.http.delete(`${this.API_URL}/api/Products/${id}`, {headers: {'Authorization': `Bearer ${this.user?.jwt}`}, responseType: 'text',observe: 'response'}).subscribe(
      res =>{
  ...
  saveNewProduct():void{
    ...
    this.http.post<ApiProduct>(`${this.API_URL}/api/Products`, newProd, {headers: {'Authorization': `Bearer ${this.user?.jwt}`}}).subscribe(
      res =>{
  ...
  editProduct(prod: Product): void{
    ...
    this.http.put<ApiProduct>(`${this.API_URL}/api/Products`, editedProd, {headers: {'Authorization': `Bearer ${this.user?.jwt}`}}).subscribe(
      res =>{
        const {id, name, description, price, categoryId, createdBy, createdAt, isArchived} = res;
  ...
  archiveProducts():void{
    ...
    this.http.patch<number[]>(`${this.API_URL}/api/Products`, archivedProductsIds, {headers:{'Authorization': `Bearer ${this.user?.jwt}`}}).subscribe(
      res=>{
